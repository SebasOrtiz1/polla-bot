...
#  v2 — Con recordatorios automáticos 8 AM Colombia
# ============================================================
...
HORA_RECORDATORIO     = 8   # 8 AM hora Colombia
...
def registrar_resultado(match_id, marcador_real):
    ...
    if match_id not in PRONOSTICOS:
        return f"❌ No encontré el partido *{match_id}*. Escribe *ayuda* para ver los IDs."
    ...
    return (f"⚽ *Resultado guardado*\n"
            f"{pron['t1']} {marcador_real} {pron['t2']}\n\n"
            f"📊 *Puntos de este partido:*\n"
            f"👤 U1 (pronóstico: {pron['u1']}) → {badge(p1)}\n"
            f"👤 U2 (pronóstico: {pron['u2']}) → {badge(p2)}\n\n"
            f"🏆 *Tabla actual:*\n"
            f"👤 U1: *{data['puntos']['u1']} pts* ({data['exactos']['u1']} exactos · {data['resultados_ok']['u1']} resultados)\n"
            f"👤 U2: *{data['puntos']['u2']} pts* ({data['exactos']['u2']} exactos · {data['resultados_ok']['u2']} resultados)\n\n"
            f"👑 Líder: *{lider}*")
...
def partidos_hoy():
    ...
    for p in partidos:
        ...
        estado = f"✅ {real}" if real else f"⏳ Pendiente → ingresa: *resultado {mid} X-X*"
        lines.append(
            f"🕐 {p['hora']} (COL) | Grupo {p['grupo']} | ID: *{mid}*\n"
            f"  {pron.get('t1','?')} vs {pron.get('t2','?')}\n"
...
def tabla_puntos():
    ...
    return (f"🏆 *Tabla de la Polla*\n\n"
            f"👤 *Usuario 1*\n"
            f"  Puntos: {data['puntos']['u1']} pts\n"
            f"  Exactos: {data['exactos']['u1']} 🎯\n"
            f"  Resultados: {data['resultados_ok']['u1']} ✅\n\n"
            f"👤 *Usuario 2*\n"
            f"  Puntos: {data['puntos']['u2']} pts\n"
            f"  Exactos: {data['exactos']['u2']} 🎯\n"
            f"  Resultados: {data['resultados_ok']['u2']} ✅\n\n"
            f"📊 Partidos ingresados: {jugados}\n"
            f"👑 Líder: *{lider}*")
...
def pronostico_partido(match_id):
    ...
    return (f"📋 *Partido {mid}*\n"
            f"{p['t1']} vs {p['t2']}\n\n"
            f"🔵 U1 pronostica: *{p['u1']}*\n"
            f"🟢 U2 pronostica: *{p['u2']}*\n"
...
def ayuda():
    return ("🤖 *Bot Polla Mundial 2026*\n\n"
            ...
            "❓ *ayuda* → Este menú\n\n"
            "💡 El ID del partido lo ves escribiendo *hoy*\n"
...
def scheduler_loop():
    """Corre en background, revisa cada minuto si son las 8 AM Colombia."""
    ultimo_envio = None
    while True:
        try:
...
# Iniciar scheduler en hilo separado al arrancar
...
def webhook():
    ...
    mensaje = " ".join(raw.strip().lower().split())
...
# ── Endpoint para probar el recordatorio manualmente ──────────
@app.route("/test-recordatorio", methods=["GET"])
def test_recordatorio():
    enviar_recordatorio()
    return "Recordatorio enviado (si hay partidos hoy)", 200
